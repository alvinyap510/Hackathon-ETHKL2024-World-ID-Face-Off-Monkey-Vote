...
const generateRandomArray = (length: number): number[] | null => {
  ...
  const getFaceOffPairsLength = (topicDetails: TopicDetail[], topicName: string): number | null => {
  ...
  const fetchMatches = async (votingTopicAddress: string, randomizedArray: number[]): Promise<any[]> => {
...
const VoteSlugPage: React.FC = () => {
  ...
  const [votingTopicAddress, setVotingTopicAddress] = useState<string | null>(null);
  ...
  useEffect(() => {
    const fetchTopics = async () => {
      try {
        ...
        if (exists) {
            const faceOffPairsLength = getFaceOffPairsLength(details, formattedSlug);
            if (faceOffPairsLength !== null) { // Check if faceOffPairsLength is not null
              const randomizedArray = generateRandomArray(Number(faceOffPairsLength));
              setRandomizedArray(randomizedArray)
            //   console.log("Randomized Array:", randomizedArray); // You can use this array as needed
              if (randomizedArray !== null) { // Check if randomizedArray is not null
                const matchedTopicIndex = details.findIndex(topic => topic.votingTopic.toLowerCase() === formattedSlug.toLowerCase());
                const votingTopicAddress = topics[matchedTopicIndex];
                setVotingTopicAddress(votingTopicAddress);

                const fetchedMatches = await fetchMatches(votingTopicAddress, randomizedArray);
...
const handleFinishVoting = (resultArray: number[]) => {
    console.log("Voting Finished! Result Array:", resultArray);
    const result = async () => await writeContract(config, {
        abi: VotingTopic.abi,
